...
// as 断言 ：  就是我说它是什么就是什么， 我确定它就是这个的意思
let arr4 = [1, "string"] as const; // 将数组确定为一个元组  
...
function nd() {
    ...
    return [a, b]; // 这样的话 ts 利用智能推断 ，将数组的 元素推断成 string | (x: number, y:number) => number;
...
const [ml, gk] = [...nd()] as const; // 所以这里的 结构出来的值的 类型也是 string | (x: number , y: number) => number

// console.log(gk(12, 88))  所以这里调用不了
//  强行调用 可以 也可以断言
// (gk as  (x:number , y: number) => number)(1, 4);
